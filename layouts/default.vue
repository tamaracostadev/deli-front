<template>
  <v-app light>
    <v-navigation-drawer
      ...
      :style="`margin-top: ${$vuetify.application.top}px`"  :value="true"
    >
      <v-list>
        <v-list-item-group  active-class="blue darken-2 text--white" >
          ...
          >
            ...
            <v-list-item-content>
              <v-list-item-title class="text-[#EEEEEE]" v-text="item.title" />
            </v-list-item-content>
          </v-list-item>
          <v-list-group
          v-else
          ...
          <template #appendIcon>
            ...
            >
            <v-list-item-icon>
              <v-icon color="grey lighten-1">{{ sub.icon }}</v-icon>
            </v-list-item-icon>
              <v-list-item-title class="text-[#EEEEEE]" v-text="sub.title" />
          </v-list-item>
          </v-list-group>
          </template>
        </v-list-item-group>
    ...
    <v-app-bar color="primary white--text" clipped-left app>
      ...
      <v-btn icon @click.stop="$vuetify.theme.dark = !$vuetify.theme.dark" >
        <v-icon>mdi-{{ `${$vuetify.theme.dark ? 'weather' : 'white-balance' }-sunny` }}</v-icon>
      </v-btn>
    </v-app-bar>
    <v-main>
      <v-container fluid>
